    ...
    m_chart_region(sf::Vector2f(w - 6 * m_margin, h - 5 * m_margin))
{
    ...
    m_y_axis.setFont(*mygui::ResourceManager::getInstance().Font());
...
void Chart::CreateAxisMarkers()
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        marker.setFont(*mygui::ResourceManager::getInstance().Font());
